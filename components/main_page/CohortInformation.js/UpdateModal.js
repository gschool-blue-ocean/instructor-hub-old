...
const UpdateModal = ({ showUpdateModal, setShowUpdateModal, onClose }) => {
  ...
  const fetchData = async () => {
    axios.get(`https://app.asana.com/api/1.0/projects/${users.cohort_asana_gid}` , {
      headers: {
    ...
    axios.get(`https://app.asana.com/api/1.0/tasks/?project=${users.cohort_asana_gid}`, {   // user cohort
      headers: {
        Authorization: `Bearer ${users.asana_access_token}`,
      }
    })
    ...
    .then(() => {      
      ...
      .then((res) => {
        console.log(res, "student data from api")
      })
    })
    // axios.get(`/api/users/${id}`)
    // .then((res) => {
    //   console.log(res, "user get response")
    // })

  }

  useEffect(() => {
    console.log(users, "user data on load")
    fetchData();
}, [])
  ...
  async function submitHandler(e) {
    ...
    await axios.get("https://app.asana.com/api/1.0/tasks/1203111662850732", {
      headers: {
        Authorization: `Bearer ${users.asana_access_token}`,  //will get logged in user's asana token
      }
    })
    .then((res) => {
...
const studentGid = studentList[0].gid //<<----LOCAL DB CALL
        ...
        headers: {
          ...
          data: { 
            data: {
              ...
              // "html_notes": `<body>${instructorNotes}\n ${cohort.studentInfo.assessmentName.toUpperCase()}: ${cohort.studentInfo.assessmentScore}</body>`, //need conditional or neeed to make this field mandatory, move to grades button
              "parent": null,
    ...
    axios({
      method:"POST",  //must be put method not patch
        ...
        headers: {
          Authorization: `Bearer ${users.asana_access_token}`,  //need template literal for ALLLLL headers so global state dependant on user <<---test next week with all members
        }, 
        data: { 
          data: {            
            "due_on": "2022-10-02",
